/**
 * Builds typed JSON for 관악구 offices that follow the head-office shape:
 *   - 관악구청장 (head)        — 3 candidates, ballot-number 3번 결번.
 *   - 관악구의원비례 (council-pr) — 3 parties as entities.
 *
 * Output layout:
 *   data/seoul/gwanak/head/{1,2,4}.json + index.json + matrix.json
 *   data/seoul/gwanak/council-pr/{1,2,5}.json + index.json + matrix.json
 *
 * 구의원 / 시의원 ship in a separate builder — their headers use a different shape.
 *
 * Run:  pnpm build:gwanak
 */
...
const SRC_ROOT =
  process.env.GWANAK_SRC ||
  path.resolve(
    process.env.HOME || "",
    "Documents/markdown_manager/files/markdown/2026-지방선거/서울특별시/관악구"
  );
const OUT_ROOT = path.resolve(__dirname, "..", "data/seoul/gwanak");
const PHOTOS_ROOT = path.resolve(__dirname, "..", "public/photos/gwanak");
...
function findPhotoForNumber(office: string, n: number): string | undefined {
  ...
  for (const ext of ["jpg", "jpeg", "png", "webp"]) {
    ...
    if (fs.existsSync(file)) return `/photos/gwanak/${office}/${padded}.${ext}`;
  }
  return undefined;
}
 ...
 *
 * - For 구청장, header is `# 기호 N번 박준희 (朴俊熙) — 더불어민주당` (mayor-shape).
 * - For 의원비례, header is `# 기호 N번 더불어민주당 — 관악구의원 비례대표`. The "name"
...
function buildHeadCandidate(filePath: string, filename: string, isPr: boolean): GwanakHeadCandidate | null {
  ...
  return {
    ...
    meta: {
      election: isPr
        ? "2026 제9회 지방선거 · 서울특별시 관악구의원 비례대표"
        : "2026 제9회 지방선거 · 서울특별시 관악구청장 후보",
...
function buildHeadIndex(
  ...
  const entries: GwanakHeadIndexEntry[] = HEAD_BALLOT_ORDER.map((n) => {
    ...
    return {
      ...
      href: `/seoul/gwanak/head/${String(c.number).padStart(2, "0")}`,
      running: true,
...
const COUNCIL_PR_BALLOT_ORDER = [1, 2, 3, 4, 5]; // 3번(혁신)·4번(개혁) 결번 가능 + 5번 정의
...
function buildCouncilPrIndex(
  ...
  // Council-pr in 관악구: only 1·2·5 filed. 3·4 are 결번 (통일기호 미출마).
  const entries = COUNCIL_PR_BALLOT_ORDER.map((n) => {
    const p = byNumber[n];
    if (!p) {
      const partyByNumber: Record<number, string> = { 3: "조국혁신당", 4: "개혁신당" };
    ...
    return {
      ...
      href: `/seoul/gwanak/council-pr/${String(p.number).padStart(2, "0")}`,
      running: true,
...
function buildDistrictCandidate(
  ...
  // Metro 시의원 H1: "# 주무열 — 서울특별시의원 관악구 제2선거구 (기호 1, 더불어민주당)"
...
function runCouncil(): {
  ...
  const officeDir = path.join(SRC_ROOT, "관악구의원");
  ...
  const idx: GwanakCouncilIndex = {
    ...
    districts: byDistrict.map(({ district, entries }) => ({
      ...
      candidates: entries.map((c) => ({
        ...
        partyColor: c.partyColor,
        district: c.district,
        href: `/seoul/gwanak/council/${district.replace("선거구", "")}/${String(c.number).padStart(2, "0")}`,
        running: true,
        wealth: c.declaration.wealth,
        criminalCount: c.declaration.criminalRecord.count,
        militaryStatus: c.declaration.military.status,
  ...
  if (matrixFile) {
    const mx = buildMatrixJson(path.join(officeDir, matrixFile), "관악구의원");
    fs.writeFileSync(path.join(outDir, "matrix.json"), JSON.stringify(mx, null, 2));
  }
...
// ---------- 관악구시의원 (metro, flat dir with filename-encoded district) ----------
...
function runMetro(): { groups: { district: string; entries: GwanakMetroCandidate[] }[] } {
  const officeDir = path.join(SRC_ROOT, "관악구시의원");
  ...
  const idx: GwanakMetroIndex = {
    ...
    districts: sortedDistricts.map((district) => ({
      ...
      candidates: groups[district].map((c) => ({
        ...
        partyColor: c.partyColor,
        district: c.district,
        href: `/seoul/gwanak/metro/${district.replace("선거구", "").replace("제", "")}/${String(c.number).padStart(2, "0")}`,
        running: true,
        wealth: c.declaration.wealth,
        criminalCount: c.declaration.criminalRecord.count,
        militaryStatus: c.declaration.military.status,
  ...
  if (matrixFile) {
    const mx = buildMatrixJson(path.join(officeDir, matrixFile), "관악구시의원");
    fs.writeFileSync(path.join(outDir, "matrix.json"), JSON.stringify(mx, null, 2));
  }
...
function main() {
  ...
  const head = runOffice("관악구청장", "head", false);
  console.log(`\n관악구청장 → ${head.candidates.length} candidates`);
  ...
  const pr = runOffice("관악구의원비례", "council-pr", true);
  console.log(`\n관악구의원비례 → ${pr.candidates.length} parties`);
  for (const c of pr.candidates) {
    console.log(`  ${c.number}. ${c.name} (party=${c.party}) · pledges=${c.pledges.length}`);
  }

  const council = runCouncil();
  console.log(`\n관악구의원 → ${council.candidates.reduce((a, d) => a + d.entries.length, 0)} candidates across ${council.candidates.length} districts`);
  ...
  console.log(`\n관악구시의원 → ${metro.groups.reduce((a, g) => a + g.entries.length, 0)} candidates across ${metro.groups.length} districts`);
