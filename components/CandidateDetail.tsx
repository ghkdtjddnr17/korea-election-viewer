...
export default function CandidateDetail({
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8">
      ...
      <header className="bg-paper border-l-8 border border-[color:var(--border)] p-6 md:p-8 rounded-sm" style={{ borderLeftColor: partyColor.bg }}>
        <div className="flex flex-col md:flex-row md:items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 text-sm">
              ...
              {candidate.header.부제 && (
                <span className="text-[color:var(--muted)]">{candidate.header.부제}</span>
              )}
            </div>
            {candidate.metaLines.length > 0 && (
              <ul className="mt-3 text-[13px] text-[color:var(--muted)] space-y-0.5">
                {candidate.metaLines.slice(0, 4).map((l, i) => (
      ...
      <div className="mt-8 flex flex-col lg:flex-row gap-8">
        ...
        <article className="flex-1 min-w-0 max-w-3xl">
          {candidate.sections.map((s, i) => (
            <section key={i} id={sectionId(s, i)} className="mb-12">
              <div className="flex items-baseline gap-3 mb-4 border-b-2 border-foreground pb-2">
                {(s.marker || s.no) && (
                  <span className="text-[11px] uppercase tracking-wider font-bold text-[color:var(--muted)] tabular-nums">
                    {s.marker || `§${s.no}`}
                  </span>
                )}
          ...
          {candidate.sourcesRaw && (
            <section id="sec-sources" className="mt-16 pt-8 border-t border-[color:var(--border)]">
              <h2 className="text-xl font-black tracking-tight mb-4">출처</h2>
              <Markdown source={candidate.sourcesRaw} />
            </section>
          )}

          <footer className="mt-12 pt-6 border-t border-[color:var(--border)] text-xs text-[color:var(--muted)]">
